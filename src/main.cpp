...
using Trajectory = typename std::map<ros::Time, Eigen::Matrix4d>;
...
DEFINE_double(alpha, 0.36, "Minimum DBoW Similarity.");
...
/// @brief voxel downsampler
beam_filtering::VoxelDownsample<> downsampler(Eigen::Vector3f(.3, .3, .3));

/// @brief Runs VPR and propsoes a set of matches between maps
/// @param map1
/// @param map2
/// @return vector of proposed matches
std::vector<vlma::VisualMapMatch> VPR(vlma::Map &map1, vlma::Map &map2);

/// @brief Performs LPR match filtering using scan context
/// @param map1
/// @param map2
/// @param matches proposed matches
/// @return relative trajectory from map2 to map1
Trajectory LPR(vlma::Map &map1, vlma::Map &map2,
               const std::vector<vlma::VisualMapMatch> &matches);

/// @brief Performs non rigid alignment to map2 given the relative trajectory
/// from LPR
/// @param relative_trajectory
/// @param map2
pcl::PointCloud<pcl::PointXYZ>::Ptr
NonRigidAlignment(const Trajectory &relative_trajectory, vlma::Map &map2);
...
int main(int argc, char *argv[]) {
  ...
  /*************************************
         VPR candidate matching
  **************************************/
  const auto initial_matches = VPR(map1, map2);

  /*************************************
         Get initial full maps
  **************************************/
  pcl::PointCloud<pcl::PointXYZ>::Ptr map1_full_cloud = map1.GetFullCloud();
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr map2_full_cloud = map2.GetFullCloud();
  ...
  /******************************************************
          LPR candidate filtering + pose estimation
  *******************************************************/
  const auto relative_trajectory = LPR(map1, map2, initial_matches);
  if (relative_trajectory.empty()) {
    BEAM_ERROR("Could not estimate a relative trajectory between maps.");
    return 1;
  }
  ...
  **************************************/
  const auto map2_full_cloud_aligned =
      NonRigidAlignment(relative_trajectory, map2);
...
Trajectory LPR(vlma::Map &map1, vlma::Map &map2,
  ...
  matcher_params.res = 0.2;
  beam_matching::GicpMatcher scan_registration(matcher_params);
  SCManager scan_context_extractor;

  Trajectory best_relative_poses;            // <map2 stamp: T_map1_map2>
  Trajectory all_relative_poses;             // <map2 stamp: T_map1_map2>
  ...
  for (const auto &match : matches) {
    const auto timestamp_map1 = match.map1_stamp;
    const auto timestamp_map2 = match.map2_stamp;
    ...
    std::cout << "DBoW score: " << match.visual_match_score << std::endl;
    std::cout << std::endl;
    i++;
  ...
  **************************************/
  if (FLAGS_filter_path_outliers) {
    ...
    Trajectory filtered_best_relative_poses;
    ...
    return filtered_best_relative_poses;
  }

  return best_relative_poses;
}

std::vector<vlma::VisualMapMatch> VPR(vlma::Map &map1, vlma::Map &map2) {
  // visual feature detector, descriptor, matcher and tracker
  ...
  for (rosbag::MessageInstance const m : bag1_view) {
    ...
    if (buffer_image) {
      const auto stamp_map = buffer_image->header.stamp;
      // we process at a max of 5hz
      if ((stamp_map - prev_frame_time).toSec() < 0.2) {
        continue;
      }
      prev_frame_time = stamp_map;

      // skip any image that isn't within the trajectory
      Eigen::Matrix4d T_WORLD_LIDAR;
      if (!map1.trajectory_lookup.GetT_WORLD_SENSOR(T_WORLD_LIDAR, stamp_map)) {
        continue;
      }
      ...
      for (int i = 0; i < kp.size(); i++) {
        map1.features.push_back(desc.row(i));
      }

      // add to database if its the first image
      if (first_image) {
        first_image = false;
        map1.images.insert({stamp_map, *buffer_image});
  ...
  *******************************************************/
  ...
  map1_vocabulary.create(map1.features);

  BEAM_INFO("Building image database for map 1.");
  boost::progress_display loading_bar_imagedb(map1.images.size());
  beam_cv::ImageDatabase image_db(detector_params, descriptor_params,
                                  map1_vocabulary);
  for (const auto &[stamp, img_msg] : map1.images) {
    ++loading_bar_imagedb;
  ...
  **************************************/
  ...
  std::vector<vlma::VisualMapMatch> matches;
  ...
  for (rosbag::MessageInstance const m : bag2_view) {
    ...
    if (buffer_image) {
      ...
      if (map1.images.find(stamp_map1) == map1.images.end()) {
        continue;
      }
      cv::Mat map1_image =
          beam_cv::OpenCVConversions::RosImgToMat(map1.images.at(stamp_map1));
      ...
      vlma::VisualMapMatch match{stamp_map2, stamp_map1, score};
...
NonRigidAlignment(const Trajectory &relative_trajectory, vlma::Map &map2) {
  ...
  std::vector<beam::Pose> relative_traj;
  for (const auto &[stamp, T_FIXED_MOVING] : relative_trajectory) {
    beam::Pose pose{T_FIXED_MOVING, stamp.toNSec()};
    relative_traj.push_back(pose);
  }
  beam::BsplineSE3 spline;
  spline.feed_trajectory(relative_traj);

  // get first and last pose rather than using the spline extrpolation
  const auto start_pose = relative_traj[0];
  const auto end_pose = relative_traj[relative_traj.size() - 1];
  ...
  return map2_full_cloud_aligned;
}
